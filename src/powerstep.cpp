//powerstep code based on Mike Hord, SparkFun Electronics
...
void PowerStep::init(){
	...
	//  calling the "PowerStep::ResetDev()" function after SPI is initialized.
	...
	//SetParam(STEP_MODE,  SYNC_EN |  SYNC_SEL_1 | VoltageMode | STEP_SEL_1_8);
	SetParam(KVAL_RUN, 128);
	SetParam(KVAL_ACC, 128);
	SetParam(KVAL_DEC, 128);
	...
	SetParam(CONFIG, CONFIG_PWM_DIV_2 | CONFIG_PWM_MUL_2 | UVLOVAL0| VCCVAL0 | CONFIG_OC_SD_DISABLE | CONFIG_VS_COMP_DISABLE | CONFIG_SW_HARD_STOP | CONFIG_INT_16MHZ);
...
boolean PowerStep::isBusy(){
	int status = getStatus();
	return !((status >> 1) & 0b1);
}

void PowerStep::setMicroSteps(int microSteps){
	  byte stepVal;
...
void PowerStep::setThresholdSpeed(float thresholdSpeed){
...
void PowerStep::setCurrent(int current){}



void PowerStep::setMaxSpeed(int speed){
...
void PowerStep::setMinSpeed(int speed){
	// Configure the MAX_SPEED register- this is the maximum number of (micro)steps per
	//  second allowed. You'll want to mess around with your desired application to see
	//  how far you can push it before the motor starts to slip. The ACTUAL parameter
	//  passed to this function is in steps/tick; MaxSpdCalc() will convert a number of
...
void PowerStep::setAcc(float acceleration){
...
void PowerStep::setDec(float deceleration){
...
void PowerStep::setOverCurrent(unsigned int ma_current){
	// Configure the overcurrent detection threshold. 
	byte OCValue = floor(ma_current / 31.25);
	if(OCValue > 0x0F)OCValue = 0x0F;
	SetParam(OCD_TH, OCValue);
}

void PowerStep::setStallCurrent(float ma_current){
...
void PowerStep::SetLowSpeedOpt(boolean enable){
...
void PowerStep::run(byte dir, float spd){
...
void PowerStep::Step_Clock(byte dir){
...
void PowerStep::move(long n_step){
...
void PowerStep::goTo_DIR(byte dir, long pos){
...
void PowerStep::goUntil(byte act, byte dir, unsigned long spd){
...
void PowerStep::releaseSW(byte act, byte dir){
...
void PowerStep::setMark(long value){
...
unsigned long PowerStep::Param(unsigned long value, byte bit_len){
	...
	// PowerStep::Xfer() sends a byte out through SPI and returns a byte received
...
unsigned long PowerStep::GetParam(byte param){
...
long PowerStep::convert(unsigned long val){
...
unsigned long PowerStep::ParamHandler(byte param, unsigned long value){
